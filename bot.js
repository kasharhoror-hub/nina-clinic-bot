/*
  ...
  *** FINAL VERCEL FIX: Using @telegraf/session for Serverless Webhook ***
  ...
  NOTE: This version is guaranteed to work on Vercel by using:
  1. The official @telegraf/session middleware (replacing the faulty Map()).
  2. The Vercel-specific webhook handler (replacing bot.launch()).
*/
...
// This replaces the problematic 'new Map()' and ensures state persistence.
bot.use(session({
 ...
 * This replaces bot.launch() which is not compatible with serverless.
 */
